...
ID_ENCUESTA=$1
API_GET_RESULTADOS="http://localhost:8000/api/reports/$ID_ENCUESTA"
DIRECCION="./exports"
...
if [ -z "$respuesta" ]; then
    echo "No se encontraron resultados para la encuesta con ID $ID_ENCUESTA."
    exit 1
fi

# Convertir resultados a CSV con preguntas como filas y respuestas como columnas
if echo "$respuesta" | jq -e '.encuestas' > /dev/null 2>&1; then
    # Obtener todas las respuestas únicas para crear las columnas
    respuestas_unicas=$(echo "$respuesta" | jq -r '.encuestas[].respuestas[]' | sort -u)
    ...
    echo "$respuesta" | jq -r '.encuestas[] | .pregunta' | sort -u | while IFS= read -r pregunta; do
        echo -n "\"$pregunta\"" >> "$CSV_FILE"
